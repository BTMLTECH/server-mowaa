...
const BACKEND_URL = process.env.BACKEND_URL || "https://server-mowaa.onrender.com";
...
export const exchangeRate = async (req: Request, res: Response) => {
  try {
    ...
    if (!rate || isNaN(rate)) throw new Error("Invalid exchange rate in environment variables");
...
export const initiatePayment = async (req: Request, res: Response) => {
  try {
    ...
    const paystackAmount = Math.round(totalAmount * 100); // convert to kobo
    ...
    const response = await axios.post(
      "https://api.paystack.co/transaction/initialize",
      {
        ...
        metadata: { name },
        callback_url: `${BACKEND_URL}/api/payment/callback` // Backend receives callback
      },
      {
        headers: {
...
export const verifyPayment = async (req: Request, res: Response) => {
  try {
    ...
    if (payment.status === "pending") {
      ...
      const data = response.data.data;
      payment.status = data.status === "success" ? "success" : data.status === "failed" ? "failed" : payment.status;
      await payment.save();
    }
